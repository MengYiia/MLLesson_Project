...
def train(model, dataset_path, batch_size, lr, Epochs, logger, save_model_dir, weight_decay,
    ...
    for epoch in range(Epochs):
        ...
        if avg_val_loss <= min_val_loss:
            ...
            logger.info('保存效果最好的模型:best_epoch_model_layer3_hideen64_torchmodel.pth')
            model_dict = model.state_dict()
            model_dict['min_val_loss'] = avg_val_loss
            torch.save(model_dict, os.path.join(save_model_dir, "best_epoch_model_layer3_hideen64_torchmodel.pth"))
...
if __name__ == '__main__':
    ...
    model = TransformerSeqPredictor(input_dim=embed_dim, hidden_dim=64, seq_len=src_len, num_heads=n_heads, num_layers=n_layers, output_dim=embed_dim)

    save_model_dir = "../model_FcTransformer_336"
